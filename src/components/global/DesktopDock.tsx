...
const DesktopDock = ({ onTerminalClick, onNotesClick, onGitHubClick, activeApps }: DesktopDockProps) => {
  ...
  const LinksPopup = () => (
    ...
    >
      <div className="grid grid-cols-1 gap-y-2">
        <a href={userConfig.social.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-gray-300 hover:text-white">
          <BsLinkedin size={20} />
          <span>LinkedIn</span>
        </a>
        <a href={userConfig.social.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-gray-300 hover:text-white">
          <BsGithub size={20} />
          <span>GitHub</span>
        </a>
        <a href={`mailto:${userConfig.contact.email}`} className="flex items-center gap-2 text-gray-300 hover:text-white">
          <IoIosMail size={20} />
          <span>Email</span>
        </a>
        <a href={`tel:${userConfig.contact.phone}`} className="flex items-center gap-2 text-gray-300 hover:text-white">
          <IoIosCall size={20} />
          <span>Téléphone</span>
        </a>
      </div>
  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 right-0 hidden md:flex justify-center pb-4 z-0">
        <div className="bg-gray-600/50 backdrop-blur-sm rounded-2xl p-2 shadow-xl">
          <div className="flex space-x-2">
            <button onClick={onGitHubClick} onMouseEnter={() => setHoveredIcon('github')} onMouseLeave={() => setHoveredIcon(null)} className="relative group">
              ...
              {hoveredIcon === 'github' && <Tooltip text='Mes Projets' />}
            </button>

            <button onClick={onNotesClick} onMouseEnter={() => setHoveredIcon('notes')} onMouseLeave={() => setHoveredIcon(null)} className="relative group">
              ...
              {hoveredIcon === 'notes' && <Tooltip text='Mes Notes' />}
            </button>

            <button onClick={handleResumeClick} onMouseEnter={() => setHoveredIcon('resume')} onMouseLeave={() => setHoveredIcon(null)} className="relative group">
              ...
              {hoveredIcon === 'resume' && <Tooltip text='Voir le CV' />}
            </button>

            <button onClick={handleCalendarClick} onMouseEnter={() => setHoveredIcon('calendar')} onMouseLeave={() => setHoveredIcon(null)} className="relative">
              ...
              {hoveredIcon === 'calendar' && <Tooltip text='Prendre un rendez-vous' />}
            </button>

            <button onClick={handleSpotifyClick} onMouseEnter={() => setHoveredIcon('spotify')} onMouseLeave={() => setHoveredIcon(null)} className="relative">
            ...
            <button onClick={handleLinksClick} onMouseEnter={() => setHoveredIcon('links')} onMouseLeave={() => setHoveredIcon(null)} className="relative">
            ...
            <button onClick={onTerminalClick} onMouseEnter={() => setHoveredIcon('terminal')} onMouseLeave={() => setHoveredIcon(null)} className="relative">
      ...
      <SpotifyPlayer isOpen={showSpotify} onClose={handleCloseSpotify} playlistId={userConfig.spotify.playlistId} />
    </>
  );
};
